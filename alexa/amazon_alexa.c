/*******************************************************************************
	...
	TIME LIST:
	CREATE By Ringsd   2016/12/20 13:50:19
...
#define AMAZON_ALEXA_KEY_PATH           "alexa.key"
...
#define AMAZON_ALEXA_KEY_PATH           "/usr/resource/alexa.key"
...
static char* cjson_parse_dup_string(cJSON* json, const char* key)
{
	...
	if( item )
	{
		...
		if( value )
		{
			...
			char* actual_value = (char*)malloc( len );
			if( actual_value )
			{
				strncpy( actual_value, value + 1, len - 2 );
				actual_value[len - 2] = 0;
			}
			free(value);
			return actual_value;
		}
...
static size_t post_response(void *buffer, size_t size, size_t nmemb, void *userp)
{
	int response_len = 0;
...
static char *curl_send_post(char *url, char *fields)
{
	int ret = 0;
	CURL *curl;
	char *response = NULL;
...
{
	struct cJSON* root_json = cJSON_CreateObject();
	struct cJSON* auth_json = cJSON_CreateObject();
	struct cJSON* token_json = cJSON_CreateObject();
	...
	aa_service_save2keyfile( root_json, AMAZON_ALEXA_KEY_PATH );
